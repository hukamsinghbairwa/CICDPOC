      ...
      <td>
        <table class="tableView" width="100%">
          ...
          <tr>
			...
			<script>
				if(is_csrf_guard_enabled && needToInsertToken) {
					document.write('<a href="javascript:document.htmlform_settings_jndi_test_jndi_alias_%value $index%.submit();"><IMG src="icons/checkdot.png" border="none"></a>');
				} else {
					document.write('<a href="settings-jndi.dsp?action=test&jndiAliasName=%value aliasName encode(url)%"><IMG src="icons/checkdot.png" border="none"></a>');
				}
			</script>			
			</d>
			...
			<script>
				if(is_csrf_guard_enabled && needToInsertToken) {
					document.write('<a href="javascript:document.htmlform_settings_jndi_delete_jndi_alias_%value $index%.submit();" onClick="javascript:return confirm(\'Are you sure you want to delete JNDI Provider Alias %value aliasName encode(javascript)%?\')"><img style="width: 13px; height: 13px;" alt="active" border="0" src="icons/delete.png"></a>');
				} else {
					document.write('<a href="settings-jndi.dsp?action=delete&jndiAliasName=%value aliasName encode(url)%" onClick="javascript:return confirm(\'Are you sure you want to delete JNDI Provider Alias %value aliasName encode(javascript)%?\')"> <img style="width: 13px; height: 13px;" alt="active" border="0" src="icons/delete.png"></a>');
				}
			</script>
              &nbsp;
            </td>
