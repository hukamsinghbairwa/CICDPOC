                 ...
                 %ifvar remote equals('no')%
                    %ifvar packageList%
                        ...
                        <script>writeTD("rowdata");</script>
							...
							<script>
								if(is_csrf_guard_enabled && needToInsertToken) {
									document.write('<a href="javascript:document.htmlform_package_unsubscribe_%value $index%.submit();"><IMG                           SRC="icons/delete.png" BORDER=0></A>');
								} else {
									document.write('<a href="package-unsubscribe.dsp?action=unsub&package=%value encode(url)%&hostport=%value publisher encode(url)%"><IMG  SRC="icons/delete.png" BORDER=0></A>');
								}
							</script>
                ...
                %else%
                    %ifvar packageList%
                        ...
                        <script>writeTD("rowdata");</script>
							...
							<script>
								if(is_csrf_guard_enabled && needToInsertToken) {
									document.write('<a href="javascript:document.htmlform_package_unsubscribe_1_%value $index%.submit();"><IMG                            SRC="icons/delete.png" BORDER=0></A>');
								} else {
									document.write('<a href="package-unsubscribe.dsp?action=unsub&package=%value encode(url)%&hostport=%value publisher encode(url)%"><IMG  SRC="icons/delete.png" BORDER=0></A>');
								}
							</script>
