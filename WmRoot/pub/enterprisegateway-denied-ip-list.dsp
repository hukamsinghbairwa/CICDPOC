        ...
        %invoke wm.server.enterprisegateway:getDeniedIPList%
            %loop deniedIPList%
		        ...
		        if(is_csrf_guard_enabled && needToInsertToken) {
		     	document.write('<a class="imagelink" href="javascript:document.htmlform_http_enterprisegateway_dos_options.submit();" onclick="return confirmDelete(\'%value encode(javascript)%\')"><img src="icons/delete.png" border="none"></a>');
		       } else {
			document.write('<a class="imagelink" href="enterprisegateway-dos-options.dsp?action=delete&IP=%value encode(url)%" onclick="return confirmDelete(\'%value encode(javascript)%\')"><img src="icons/delete.png" border="none"></a>');
		     }
           </script>
                    </td>
                </tr>
