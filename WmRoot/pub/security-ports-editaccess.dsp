      ...
      <TD width=100%>
       ...
       <TR>
        ...
        <TR>
			...
			<script>
				if(is_csrf_guard_enabled && needToInsertToken) {
					document.write('<a href="javascript:document.htmlform_security_ports_editaccess_delete.submit();" onClick="return populateForm(document.htmlform_security_ports_editaccess_delete, \'port=%value port%;node=%value%\')"><img src="icons/delete.png" border="no"></a>');
				} else {
					document.write('<a href="security-ports-editaccess.dsp?port=%value port encode(url)%&node=%value encode(url)%&Action=deleteNode&Page=Edit"><img src="icons/delete.png" border="no"></a>');
				}
			</script>
