%invoke wm.server.ws:listProviderEndpoints%
    ...
    <script>writeTD("rowdata-l");</script>
	  ...
	  <script>
		if(is_csrf_guard_enabled && needToInsertToken) {
			document.write('<a class="imagelink" href="javascript:document.htmlform_wsendpoints_listProviders_%value $index%.submit();" onClick="return confirmDelete(\'%value alias encode(javascript)%\',\'provider\', \'%value transportInfo/transportType encode(javascript)%\');"> <img src="icons/delete.png" alt="Delete alias : %value alias encode(htmlattr)%" border="none"></a>');
		} else {
			document.write('<a class="imagelink" href="" onClick="return confirmDelete(\'%value alias encode(javascript)%\',\'provider\', \'%value transportInfo/transportType encode(javascript)%\');"> <img src="icons/delete.png" alt="Delete alias : %value alias encode(htmlattr)%" border="none"></a>');
		}
	</script>
