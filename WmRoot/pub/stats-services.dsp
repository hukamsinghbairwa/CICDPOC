            ...
            <TR>
               <TD>
		        ...
		        if(is_csrf_guard_enabled && needToInsertToken) {
		     	document.write('<A href="javascript:document.htmlform_service_info_%value $index%.submit();"> %value ifc encode(html)%:%value svc encode(html)%</A>');
		       } else {
			document.write('<A href="service-info.dsp?service=%value ifc encode(url)%:%value svc encode(url)%"> %value ifc encode(html)%:%value svc encode(html)%</A>');
		     }
           </script>
